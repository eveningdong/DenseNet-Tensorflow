...
from libs.datasets.cifar import cifar10_input as cifar10
...
def main():
  ...
  # Train input
  images, labels = cifar10.distorted_inputs(data_dir=args.data_dir,
  ...
  # Validation
  images_val, labels_val = cifar10.inputs(data_dir=args.data_dir,
                                          batch_size=100,
                                          is_training=False)
  ...
  num_epochs = 300
  num_steps = num_epochs * cifar10.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN // \
    args.batch_size
  ...
  for step in range(args.ckpt, args.num_steps):
    ...
    if step % args.save_pred_every == 0 and step > args.ckpt:
      ...
      # for val_step in range(cifar10.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL//100):
